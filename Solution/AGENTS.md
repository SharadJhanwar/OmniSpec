...
| **8** | **Digital Asset Synthesizer** | Discovers real product image URLs via DuckDuckGo Image Search; falls back to canonical `<Brand>_<MPN>.jpg` filenames if unavailable. | DuckDuckGo Image Search API, marketplace filter (blocks Amazon/eBay), string formatters, MIME validators. | Visual asset relevance scorer; source quality filter. | `Product Image` (real URL or canonical), `Alternate Image 1..4`, `Specification Sheet`, `SDS`, `RoHS`, `Actual Image (Yes/No)`. |
