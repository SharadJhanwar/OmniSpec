...
from ..schemas.family_schema import (
    VariantAxis,
    ChildVariant,
    AssortmentGap,
    ParentProductFamily,
    FamilyDiscoveryResponse
)


class FamilyClusteringEngine:
    """
    Deterministic-First Product Family Discovery & Assortment Gap Detection Engine.
    Clusters flat catalog SKUs into canonical Parent PDPs and identifies missing contractor sizes.
    """
    ...
    def detect_assortment_gaps(
        ...
        present_sizes = set()
        for v in variants:
            desc = v.short_desc.lower()
            axis_vals = v.axis_values
    ...
    @classmethod
    def discover_product_families(cls, catalog_items: List[Dict[str, Any]]) -> FamilyDiscoveryResponse:
        """
        Discovers all Parent Product Families and Assortment Gaps across catalog items.
        """
        ...
        families: List[ParentProductFamily] = []
        all_gaps: List[AssortmentGap] = []
        total_clustered_skus = 0
        ...
        for group_key, members in groups.items():
            ...
            for it, extracted_axes in members:
                ...
                child_variants.append(ChildVariant(
                    mpn=v_mpn,
                    brand_name=brand,
                    manufacturer_name=it.get("MANUFACTURER_NAME", "") or it.get("manufacturer_name", ""),
                    short_desc=v_desc,
                    axis_values=extracted_axes,
                    image_url=it.get("Product Image", "") or it.get("product_image", "")
                ))

            variant_axes = [
                VariantAxis(name=name, values=sorted(list(vals)))
                for name, vals in axis_collector.items()
            ]

            gaps = cls.detect_assortment_gaps(brand, base_series, cat_path, child_variants)
            all_gaps.extend(gaps)
            total_clustered_skus += len(child_variants)

            families.append(ParentProductFamily(
                family_id=f"fam_{brand.lower().replace(' ', '_')}_{base_series.lower().replace(' ', '_')}",
                family_name=family_name,
                brand_name=brand,
                category_path=cat_path,
                base_series_mpn=base_series,
                total_variants=len(child_variants),
        ...
        return FamilyDiscoveryResponse(
            total_families_discovered=len(families),
            total_child_skus_clustered=total_clustered_skus,
            families=families,
            all_gaps=all_gaps,
